...
1. **Setup**: Ensure local `main` is up-to-date with remote. Pull if necessary. Abort if there are uncommitted changes that conflict with the pull.
2. **Version Logic**: Bump the version based on `$ARGUMENTS` (defaults to `patch` if not specified). Examples: if current is `2.1.0`, then `patch` → `2.1.1`, `minor` → `2.2.0`, `major` → `3.0.0`. If the bump is `minor` or `major`, show the user the current and new version and ask for confirmation before proceeding.
...
7. **Delivery**: Push the branch and open a Pull Request using the GitHub CLI (`gh pr create`). Use the user signature; do not co-author the commits. Use the following PR format:
   ...
   - Body:
     ```
     ## Summary
     - Bump version from {old_version} to {version_number}
     ```
