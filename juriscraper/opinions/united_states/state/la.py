...
class Site(OpinionSite):
    def __init__(self, *args, **kwargs):
        ...
        self.url = "http://www.lasc.org/news_releases/{year}/default.asp".format(
    ...
    def _add_judges(self, html_tree):
        judges = []
        for e in html_tree.xpath(
            "//strong[contains(., 'J.') or contains(., 'CURIAM')]"
        ):
    ...
    def _add_dates(self, html_tree):
        ...
        text = " ".join(
            list(
                html_tree.xpath(
                    "//text()[contains(normalize-space(), 'day of')]"
                )
            )
        )
        text = " ".join(text.split())
        if text:
            match = re.search(
                r"\D*((\d{1,2})\w{2} day of (\w+),\s*(\d{4})).*", text
            )
            date_text = "{day}{month}{year}".format(
                day=match.group(2), month=match.group(3), year=match.group(4)
            )
            case_date = date.fromtimestamp(
                time.mktime(time.strptime(date_text, "%d%B%Y"))
            )
            dates.extend(
                [case_date] * self._get_link_count_from_html(html_tree)
            )
        return dates
