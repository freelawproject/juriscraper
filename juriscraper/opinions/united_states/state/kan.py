...
class Site(OpinionSite):
    ...
    def _download(self, request_dict={}):
        r = self.request["seesion"].get(self.url)
        ...
        while self.page:
            ...
            r = self.request["seesion"].post(self.url, data=self.data)
            self.soup = fromstring(r.text)
            for row in self.soup.xpath(
                './/td/a[@class="link-pdf"]/ancestor::tr'
            ):
                self.rows.append(row)
            last_date_str = (
                self.soup.xpath(date_regex)[-1].text_content().strip()
            )
    ...
    def _get_case_names(self):
        return [
            x.xpath(".//td[3]")[0].text_content().strip() for x in self.rows
        ]
