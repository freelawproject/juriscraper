...
class Site(OpinionSite):
        ...
        for url in html_l.xpath(path):
            ...
            for e in html_tree.xpath("//a"):
                if (
                    ...
                    and e.attrib["href"] == previous_a.attrib["href"]
                ):
                    # Same. Merge their text then delete the extraneous element.
                    previous_text = html.tostring(
                        previous_a, method="text", encoding="unicode"
                    )
                    e_text = html.tostring(
                        e, method="text", encoding="unicode"
                    )
                    previous_a.text = " ".join(
                        (previous_text + e_text).split()
                    )
    ...
    def _return_download_urls(self, html_tree):
        return [
            a.xpath("@href")[0]
            for a in self._return_anchors_with_text(html_tree)
        ]
    ...
    def _return_anchors_with_text(self, html_tree):
        return [
            a
            for a in html_tree.xpath(self.base_path)
            if a.text_content().strip()
        ]

    def sanitize_text(self, text):
        """Prevent non-standard characters and typos from breaking regex"""
        return self.fix_court_year_id_typo(
            clean_string(normalize_dashes(text))
        )
