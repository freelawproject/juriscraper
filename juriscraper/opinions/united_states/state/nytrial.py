...
class Site(OpinionSiteLinear):
    ...
    def extract_from_text(self, scraped_text: str) -> Dict[str, Any]:
        ...
        Limit to first 3000 characters to avoid picking a citation or
        docket number from opinion's argumentation. All values we are parsing
        appear on summary tables at the beginning of the document, and
        sometimes may not exist
        """
        scraped_text = scraped_text[:3000]

        pattern = (
            r"<td[^>]*>(.*?),\s?[JS]\.</td>|Judge:\s?(.+)|([\w .,]+), J\.\s+"
        )
        ...
        if scraped_text.find("<table") != -1:
            ...
            full_case = fromstring(scraped_text).xpath("//table[2]")
            full_case = full_case[0].text_content() if full_case else ""
