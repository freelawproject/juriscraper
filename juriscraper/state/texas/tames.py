...
class TAMESScraper(BaseStateScraper):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.seen_case_urls: set[str] = set()
            ...
            for item in search_gen:
                ...
                if case_date_str:
                    try:
                        ...
                        pass
                if case["case_url"] not in self.seen_case_urls:
                    self.seen_case_urls.add(case["case_url"])
                    new_cases += 1
                    yield case
                else:
                    redundant_cases += 1
    ...
    def _parse_search_results(
        ...
        for row in rows:
            ...
            # Extract text content from each cell, handling &nbsp; as empty
            ...
            yield TamesSearchRow(
                ...
                case_url=case_url,
                date_filed=get_cell_text(cells[1]),
                style=get_cell_text(cells[2]),
                v=get_cell_text(cells[3]),
                case_type=get_cell_text(cells[4]),
                coa_case_number=get_cell_text(cells[5]),
                trial_court_case_number=get_cell_text(cells[6]),
                trial_court_county=get_cell_text(cells[7]),
                trial_court=get_cell_text(cells[8]),
                appellate_court=get_cell_text(cells[9]),
                court_code=get_cell_text(cells[10]),
            )
