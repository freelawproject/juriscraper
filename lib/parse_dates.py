...
def parse_dates(s, debug=False, sane_start=1750, sane_end=2050):
    '''Parse dates out of a string

    Based on http://stackoverflow.com/questions/7028689/, this method is
    ...
    for item in timesplit(s):
        ...
        try:
            ...
            if date.year != DEFAULT.year and \
                    (date.month != DEFAULT.month and date.day != DEFAULT.day):
                if debug:
                    print "Item %s parsed as: %s" % (item, date)
                if sane_start < date.year < sane_end:
                    dates.append(date)
