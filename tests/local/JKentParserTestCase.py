"""Test harness for JKentParser implementations.

Coverage is discovering by convention. Every
concrete ``JKentParser`` subclass is found via ``__subclasses__`` (after
...
def _unwrap_deferred(obj: object) -> object:
    """Recursively replace DeferredValidation wrappers with their raw_data.

    Parsers defer validation top-to-bottom: a deferred parent may carry
    deferred children (attorneys, docket_entries, files). ``raw_data``
    only unwraps one level, so recurse before serializing.
    """
    if isinstance(obj, DeferredValidation):
        return _unwrap_deferred(obj.raw_data)
    if isinstance(obj, dict):
        return {k: _unwrap_deferred(v) for k, v in obj.items()}
    if isinstance(obj, (list | tuple)):
        return [_unwrap_deferred(v) for v in obj]
    return obj


class JKentParserTestCase(unittest.TestCase):
    """A mixin to add JKentParser fixture testing."""

    def assert_kent_parsers_have_examples(self) -> None:
        """Run every discovered ``JKentParser`` against its fixtures.

        Imports every ``parsers.py`` under ``juriscraper/`` so that all
        concrete ``JKentParser`` subclasses are registered, then enforces
        ...
        * Reverse — each example subdirectory under a scraper's examples
        ...
        parsers = sorted(
            (
                cls
                for cls in self._all_subclasses(JKentParser)
                if not inspect.isabstract(cls)
            ),
            key=lambda c: (c.__module__, c.__name__),
        )
        self.assertTrue(
            parsers, "Discovered no concrete JKentParser subclasses to test"
        )
        ...
        for root, expected in sorted(expected_by_root.items()):
            ...
            with self.subTest(
                ...
                self.assertEqual(
                    set(),
                    actual - expected,
                    f"Example dirs under {root} name no JKentParser: "
    ...
    def _run_one(
        ...
        actual = [
            pydantic_core.to_jsonable_python(_unwrap_deferred(dv))
            for dv in results
        ]
